...
function EntryRow({ entry, view, onTag, onChanged }: { entry: TableEntry; view: View; onTag: (tag: string) => void; onChanged: () => void }) {
...
function ActivityRun({ entries, onTag, onChanged }: { entries: TableEntry[]; onTag: (tag: string) => void; onChanged: () => void }) {
  const [expanded, setExpanded] = useState(false)
  const [first, ...rest] = entries
  if (!first) return null
  return (
    <>
      <EntryRow entry={first} view="activity" onTag={onTag} onChanged={onChanged} />
      {rest.length > 0 && !expanded && (
        <li className="entry-run-more">
          <button type="button" className="link-button" onClick={() => setExpanded(true)}>
            +{rest.length} more from {first.source} on {first.project_name}
          </button>
        </li>
      )}
      {expanded && rest.map((entry) => <EntryRow key={entry.id} entry={entry} view="activity" onTag={onTag} onChanged={onChanged} />)}
    </>
  )
}
...
function EntriesView({ view, initialProject }: { view: Exclude<View, 'projects'>; initialProject: string }) {
  ...
  const groups = useMemo(() => groupBy(entries, (entry) => (view === 'todos' ? entry.project_name : dayLabel(entry.created_at))), [entries, view])
      ...
      {groups.map((group) => (
        <section key={group.key} className="entry-group" aria-label={group.key}>
          ...
          <ul className="entry-list">
            {view === 'activity'
              ? groupBy(group.items, (entry) => `${entry.slug}\u0000${entry.source}`).map((run) => <ActivityRun key={run.items[0]?.id} entries={run.items} onTag={(tag) => set('tag', tag)} onChanged={table.reload} />)
              : group.items.map((entry) => <EntryRow key={entry.id} entry={entry} view={view} onTag={(tag) => set('tag', tag)} onChanged={table.reload} />)}
          </ul>
        </section>
      ))}
      ...
      <tbody>
        {projects.map((project) => (
          <tr key={project.slug}>
            ...
            <td data-label="Latest status"><div>
              {project.status_at ? (
                <>
                  <span className="clamp status-text">{project.status_title || project.status_body}</span>
                  <span className="muted small"><Timestamp iso={project.status_at} /> · {project.status_source}</span>
                </>
              ) : <span className="muted">No status yet</span>}
            </div></td>
