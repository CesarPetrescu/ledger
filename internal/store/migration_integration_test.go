...
func TestMigrationsEmptyAndIdempotent(t *testing.T) {
	...
	var versions []int
	if err := db.Pool.QueryRow(ctx, `SELECT array_agg(version ORDER BY version) FROM schema_migration`).Scan(&versions); err != nil || !reflect.DeepEqual(versions, []int{1, 2, 3, 4, 5, 6, 7, 8}) {
