...
# These placeholders are used only to parse Compose before hashing.
...
cleanup() {
  ...
  rm -f /tmp/ledger-glass-ci.crt
  rm -rf "$GLASS_RUNTIME_DIR"
  exit "$code"
}
trap cleanup EXIT
# Do not leak node_modules/test output into Go image build contexts.
...
sudo install -m 644 "$GLASS_CERT_DIR/localhost.crt" /usr/local/share/ca-certificates/ledger-glass-ci.crt
sudo update-ca-certificates >/dev/null
mkdir -p "$HOME/.pki/nssdb"
if [[ ! -f "$HOME/.pki/nssdb/cert9.db" ]]; then certutil -N -d "sql:$HOME/.pki/nssdb" --empty-password; fi
certutil -A -d "sql:$HOME/.pki/nssdb" -n ledger-glass-ci -t C,, -i "$GLASS_CERT_DIR/localhost.crt"
...
# Public, tracked source only. Never include .env, runtime secrets, or simulator storage.
...
# Silence is a virtual audio device, not a fake transcription result.
